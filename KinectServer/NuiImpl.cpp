...
{      
	...
	printf("%04.3f %04.3f | %04.3f %04.3f\r",
		...
		skel.SkeletonPositions[NUI_SKELETON_POSITION_SHOULDER_RIGHT].x,
		skel.SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_LEFT].x,
		skel.SkeletonPositions[NUI_SKELETON_POSITION_SHOULDER_LEFT].x
		);
	if (skel.SkeletonPositions[NUI_SKELETON_POSITION_WRIST_RIGHT].y > skel.SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_RIGHT].y) {
		// 手の角度
		angle = atan2(
			skel.SkeletonPositions[NUI_SKELETON_POSITION_WRIST_RIGHT].y - skel.SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_RIGHT].y,
			skel.SkeletonPositions[NUI_SKELETON_POSITION_WRIST_RIGHT].x - skel.SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_RIGHT].x
		);
		printf("R:ANGL: %04.3f          \r",angle*180/M_PI);
		udpSend(angle*180/M_PI);
	}
	else if (skel.SkeletonPositions[NUI_SKELETON_POSITION_WRIST_LEFT].y > skel.SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_LEFT].y) {
		// 手の角度
		angle = atan2(
			skel.SkeletonPositions[NUI_SKELETON_POSITION_WRIST_LEFT].y - skel.SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_LEFT].y,
			skel.SkeletonPositions[NUI_SKELETON_POSITION_WRIST_LEFT].x - skel.SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_LEFT].x
		);

		printf("L:ANGL: %04.3f          \r",angle*180/M_PI+180);
		udpSend(angle*180/M_PI+180);
	}
	else {
		udpSend(180);
	}
	...
	/*
	//printf("POS: %04.3f\t%04.3f\t%04.3f\t%04.3f\r",skel.SkeletonPositions[3].x,skel.SkeletonPositions[3].y,skel.SkeletonPositions[3].z,skel.SkeletonPositions[3].w);
	float angle = atan2(skel.SkeletonPositions[NUI_SKELETON_POSITION_SPINE].y,skel.SkeletonPositions[NUI_SKELETON_POSITION_SPINE].z) * 180/M_PI;
			//printf("%04f\n",abs(lastAngle - angle));

	//	printf("ANGL: %04.3f\r",angle*180/M_PI);
	long currCameraAngle;
	HRESULT currAngleRes = m_pNuiSensor->NuiCameraElevationGetAngle(&currCameraAngle);
	if (currAngleRes == S_OK)
	{
		if (abs((float)currCameraAngle - lastTargetAngle) <= 2 || abs((float)currCameraAngle - angle) <= 2) {
			//printf("[TRGT DETERMINED] %04.1f\n",angle);
			...
			if (camMoving == false)
			{
				camMoving = true;
				printf("=== MOVE START ===\n");
				HRESULT elevRes = m_pNuiSensor->NuiCameraElevationSetAngle((long)floor(lastTargetAngle));
				if (elevRes == ERROR_RETRY)
				{
					printf("[CAM] %02ld -- [TRGT]: %02ld\n",currCameraAngle,(long)floor(lastTargetAngle));
				}
				else if (elevRes == ERROR_TOO_MANY_CMDS)
				{
					printf("[CAM] %02ld    [TRGT]: %02ld\n",currCameraAngle,(long)floor(lastTargetAngle));
				}
				else if (elevRes == S_OK) {
					printf("[CAM] %02ld -> [TRGT]: %02ld\n",currCameraAngle,(long)floor(lastTargetAngle));
				}
				else {
					printf("[CAM] %02ld xx [TRGT]: %02ld\n",currCameraAngle,(long)floor(lastTargetAngle));
				}
			}
		}
